...
### Instrumental variables with a panel dataset - basic syntax
  data("Crime", package = "plm")
  reg1 =  lm(log(crmrte)~ log(polpc)+log(density) + factor(year) , data = Crime)
  reg2 =  plm(formula= log(crmrte)~ log(polpc)+log(density) + factor(year) | . -log(polpc)+  log(taxpc), data = Crime,model = "within")
  reg3 =  plm(formula= log(crmrte)~ log(polpc)+log(density) + factor(year) | . -log(polpc)+  log(taxpc), data = Crime,model = "random")

  stargazer(reg1, reg2, reg2, reg3,
            title="IV Regression", type="text", 
            df=FALSE, digits=5)
  
  # See website for info on SE
